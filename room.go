...
func (d Date) IsBetween(start Date, end Date) bool {
	return (d.After(start) || d.Equal(start)) && (d.Before(end) || d.Equal(end))
}
...
func (h Hotel) getBookedRooms(arrival Date, departure Date) []Room {
	...
	for _, booking := range h.bookings {
		if booking.arrival.IsBetween(arrival, departure) || booking.departure.IsBetween(arrival, departure) {
